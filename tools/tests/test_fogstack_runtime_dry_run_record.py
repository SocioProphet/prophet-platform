...
def build_inputs(tmp_path: Path) -> tuple[Path, Path]:
    ...
    subprocess.run([sys.executable, "tools/build_fogstack_local_cluster_runtime_adapter.py", "--node-profile", str(node_profile), "--deploy-plan", str(deploy_plan), "--cluster-readiness-record", str(cluster_record), "--gitops-bundle", str(gitops_dir / "gitops-bundle.json"), "--gitops-readiness-record", str(gitops_record), "--output", str(adapter)], check=True)
...
def test_emit_fogstack_runtime_dry_run_record(tmp_path: Path) -> None:
    ...
    assert {artifact["id"] for artifact in record["artifacts"]} == {"runtime-adapter", "node-profile", "deploy-plan", "cluster-readiness-record", "gitops-bundle", "gitops-readiness-record", "kubernetes-configmap", "kubernetes-deployment", "kubernetes-service"}


def test_emit_fogstack_runtime_dry_run_record_rejects_tampered_input(tmp_path: Path) -> None:
    adapter, manifests = build_inputs(tmp_path)
    data = load_json(adapter)
    data["inputs"]["node_profile_digest"] = "sha256:" + ("0" * 64)
