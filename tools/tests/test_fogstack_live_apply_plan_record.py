...
def run_plan(tmp_path: Path, status: str, *extra: str) -> subprocess.CompletedProcess[str]:
    ...
    output = tmp_path / "plan-record.json"
...
def test_plan_blocks_when_preflight_is_blocked(tmp_path: Path) -> None:
    proc = run_plan(tmp_path, "blocked")
    assert proc.returncode == 0, proc.stderr
    record = read_json(tmp_path / "plan-record.json")
...
def test_plan_passes_when_preflight_passes(tmp_path: Path) -> None:
...
def test_require_ready_fails_when_preflight_is_blocked(tmp_path: Path) -> None:
