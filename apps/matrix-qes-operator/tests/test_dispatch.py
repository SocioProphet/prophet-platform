...
def test_transcript_extract_custom_since(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "--since" in launched[0]
    assert "48" in launched[0]


def test_transcript_extract_popen_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    def _bad_popen(cmd: list[str], **kw: object) -> None:
        raise FileNotFoundError("turtle-transcript-extract not found")

    monkeypatch.setattr("app.dispatch.subprocess.Popen", _bad_popen)
    result = execute(_cmd("transcript", "extract"))
    assert not result.ok
    assert "⚠" in result.body
    ...
    records = [
        {
            ...
            "claim": f"Fact number {i}: something useful",
            "source": "matrix",
        }
        for i in range(1, 6)
    ]
...
def test_transcript_list_caps_at_10(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pending = tmp_path / "pending.jsonl"
    records = [
        {"id": f"claim-{i:08x}", "extracted_at": "2026-08-04T12:00:00Z", "claim": f"Fact {i}"}
        for i in range(1, 20)
    ]
    pending.write_text("\n".join(json.dumps(r) for r in records) + "\n")
    monkeypatch.setattr("app.dispatch._transcript_pending_path", lambda: pending)
    ...
    result = execute(_cmd("transcript", "list"))
    assert result.ok
    assert "19 total" in result.body
...
def test_transcript_commit_runs_bin(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.dispatch.subprocess.run",
        lambda *a, **kw: _fake_run_ok("5 claim(s) committed"),
    )
...
def test_transcript_commit_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.dispatch.subprocess.run",
        lambda *a, **kw: _fake_run_fail(stderr="kb dir not found"),
    )
    result = execute(_cmd("transcript", "commit"))
    assert not result.ok
    assert "⚠" in result.body
