...
def validate_request(payload):
    validate_zone_publication_request, _, _, _ = _load_semantic_bridge_validators()
    return validate_zone_publication_request(payload)


def validate_plan(payload):
    _, validate_zone_publication_plan, _, _ = _load_semantic_bridge_validators()
    return validate_zone_publication_plan(payload)


def validate_record(payload):
    _, _, validate_zone_publication_record, _ = _load_semantic_bridge_validators()
    return validate_zone_publication_record(payload)


def validate_outcome(payload):
    _, _, _, validate_zone_publication_outcome = _load_semantic_bridge_validators()
    return validate_zone_publication_outcome(payload)
