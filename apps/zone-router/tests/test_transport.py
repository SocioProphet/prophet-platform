...
from zone_router.transport import write_publication_outcome
...
def test_write_publication_outcome_writes_outcome_and_latest(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("SOCIOPROFIT_STATE_HOME", str(tmp_path / "state"))
    record = _make_record(tmp_path)

    result = write_publication_outcome(record)
    ...
    assert result["outcome"]["topic"] == "zone.edge.carrier.ingested.v1"
