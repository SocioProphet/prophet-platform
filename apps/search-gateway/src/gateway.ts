 ...
 *   - sherlock-engine  (ontology-driven Discovery) → source: "corpus"   (Tantivy/Rust full-text over curated corpora)
 *
...
export type Source = 'web' | 'commons' | 'corpus'
...
const CORPUS_URL = process.env['SHERLOCK_URL'] ?? 'http://sherlock-engine.socioprophet.svc.cluster.local:8093'
...
async function searchCorpus(query: string): Promise<SearchResult[]> {
  const u = `${CORPUS_URL.replace(/\/$/, '')}/search?q=${encodeURIComponent(query)}&limit=${PER_SOURCE_LIMIT}`
  const data = (await getJson(u)) as { hits?: Array<{ id?: string; title?: string; snippet?: string; doctype?: string }> }
  return (data.hits ?? [])
    .slice(0, PER_SOURCE_LIMIT)
    .map((r) => ({
      title: r.title ?? '',
      url: `sherlock://${r.id ?? ''}`,
      snippet: (r.snippet ?? '').replace(/<[^>]+>/g, ''), // strip highlight tags for the synthesizer
      source: 'corpus' as const,
      engine: `tantivy${r.doctype ? '/' + r.doctype : ''}`,
    }))
}

/** Fan out, blend, de-dupe by url. Sovereign sources (commons + corpus) lead — the differentiator — then web. */
export async function blendedSearch(query: string): Promise<BlendedResults> {
  const q = String(query ?? '').trim()
  if (!q) return { query: '', results: [], counts: { web: 0, commons: 0, corpus: 0 } }

  const [commonsR, corpusR, webR] = await Promise.allSettled([searchCommons(q), searchCorpus(q), searchWeb(q)])
  const commons = commonsR.status === 'fulfilled' ? commonsR.value : []
  const corpus = corpusR.status === 'fulfilled' ? corpusR.value : []
  const web = webR.status === 'fulfilled' ? webR.value : []

  const seen = new Set<string>()
  const results: SearchResult[] = []
  for (const r of [...commons, ...corpus, ...web]) {
  ...
  const degraded: { web?: string; commons?: string; corpus?: string } = {}
  if (commonsR.status === 'rejected') degraded.commons = String((commonsR.reason as Error)?.message ?? 'commons unavailable')
  if (corpusR.status === 'rejected') degraded.corpus = String((corpusR.reason as Error)?.message ?? 'corpus unavailable')
  if (webR.status === 'rejected') degraded.web = String((webR.reason as Error)?.message ?? 'web unavailable')

  return {
    query: q,
    results,
    counts: { web: web.length, commons: commons.length, corpus: corpus.length },
