...
The checker must validate at least one of:

- an explicit invariant proof object; or
- deterministic replay output that reproduces `PROVED` under pinned domains and budgets.
...
```python
def check(artifact, events, policy_bundle, config):
    assert schema_ok(artifact)
    assert all(event_schema_ok(e) for e in events)
    assert artifact.schema_version in allowlisted_schema_versions
    assert verify_policy_bundle(policy_bundle, artifact.policy_bundle)
    assert artifact.compiler_id in allowlisted_compilers[artifact.claim.kind]
    assert recompute_inputs_hash(events, config) == artifact.inputs_hash
    assert coverage_ok_or_not_proved(artifact, events)

    if artifact.result == "PROVED":
        assert assumptions_allow_proved(artifact.assumptions)
        assert not budgets_exhausted(artifact)
        assert proved_payload_ok(artifact, events, policy_bundle)
    elif artifact.result == "VIOLATION":
    ...
    elif artifact.result == "INCONCLUSIVE":
        assert uncertainty_declared(artifact)
    else:
