...
.PHONY: engine-guards validate validate-repo validate-regis-acr-integration smoke-regis-acr-service validate-provable-ai-ops-exchange docs-check drift-check standards-check validate-argocd-source-sovereignty controls-census-gate validate-workflow-deploy-honesty topology-check chronos-evidence-loop-readout-validate lattice-surfaces-check lattice-surface-ingestor-smoke lattice-studio-smoke grlplus-service-smoke grl-mesh-smoke sophos-reasoner-smoke entity-resolution-smoke validate-ops-fabric validate-search-academy-deploy validate-search-image-release validate-lampstand-lifecycle validate-zone-stack-audit policy-fabric-endpoint-client-smoke policy-fabric-guarded-workflow-smoke zone-router-publication-local-publish-smoke zone-router-publication-failure-evidence-smoke zone-router-publication-retry-state-smoke zone-router-publication-remote-broker-seam-smoke zone-router-publication-dead-letter-smoke validate-workroom-update-contract validate-professional-intelligence-manifest validate-wallguard-professional-workroom validate-wallguard-professional-workroom-runtime validate-svf-agent-contract validate-live-sociosphere-svf-contract validate-fogstack-svf-signadot-adapter-readiness validate-environment-validate-change-v2 validate-trust-chain-contracts validate-channel-runtime-gates test-go test-python-apps test-tools smoke smoke-health smoke-eval-fabric smoke-evidence-receipts smoke-evidence-console validate-phase3 lampstand-smoke validate-phase4 lampstand-vertical-slice-smoke lampstand-zone-smoke zone-router-publication-smoke zone-router-publication-enqueue-smoke semantic-bridge-zone-validation-smoke validate-fogstack validate-storage-suite trustops-art-runner-smoke validate-workspace-services test-workspace smoke-workspace workspace-up workspace-down workspace-logs workspace-build prometheus-local-demo validate-workroom-scope-d-adversarial devsecops-workroom-demo validate-proof-artifacts validate-evidence-cover-graph check-cover-sufficiency validate-adr-035-contracts validate-helper-causal-receipts validate-svc-substrate-source-control validate-systema-bridge validate-workroom-schemas validate-device-orchestration validate-mutation-evidence validate-fogstack-svf-p2-evidence-gates validate-repo-governance-contracts validate-semantic-governance validate-orggov-runtime-demo validate-vector-chunk-contract
validate: validate-repo validate-regis-acr-integration validate-workspace-prophet-membrane-e2e validate-provable-ai-ops-exchange drift-check standards-check validate-argocd-source-sovereignty controls-census-gate validate-workflow-deploy-honesty topology-check chronos-evidence-loop-readout-validate lattice-surfaces-check lattice-surface-ingestor-smoke lattice-studio-smoke grlplus-service-smoke grl-mesh-smoke sophos-reasoner-smoke entity-resolution-smoke validate-ops-fabric validate-search-academy-deploy validate-search-image-release validate-lampstand-lifecycle validate-zone-stack-audit policy-fabric-endpoint-client-smoke policy-fabric-guarded-workflow-smoke zone-router-publication-local-publish-smoke zone-router-publication-failure-evidence-smoke zone-router-publication-retry-state-smoke zone-router-publication-remote-broker-seam-smoke zone-router-publication-dead-letter-smoke validate-workroom-update-contract validate-professional-intelligence-manifest validate-wallguard-professional-workroom validate-wallguard-professional-workroom-runtime validate-svf-agent-contract validate-live-sociosphere-svf-contract validate-fogstack-svf-signadot-adapter-readiness validate-environment-validate-change-v2 validate-trust-chain-contracts validate-channel-runtime-gates test-go validate-phase4 test-python-apps test-tools validate-fogstack validate-storage-suite trustops-art-runner-smoke validate-workroom-scope-d-adversarial devsecops-workroom-demo validate-proof-artifacts validate-evidence-cover-graph check-cover-sufficiency validate-adr-035-contracts validate-helper-causal-receipts validate-svc-substrate-source-control validate-systema-bridge validate-workroom-schemas validate-device-orchestration validate-mutation-evidence validate-semantic-governance validate-orggov-runtime-demo validate-vector-chunk-contract
...
# Workflow deploy-honesty: no deploy workflow may swallow a cluster mutation's failure
# (`kubectl apply … || true`). Generalizes the 2026-08-04 orphan-gitea lesson — a deploy control
# that reports green while broken is a paper control. Static; shrink-only ratchet; self-testing.
validate-workflow-deploy-honesty:
	python3 tools/verify_workflow_deploy_honesty.py
...
# The gate form of the census (P3): fail if any NEW scheduled control lacks a negative control.
# Shrink-only ratchet — the two current guards are allowlisted (KNOWN_UNDISCRIMINATING) and can
# only be removed by giving them a self-test. Keeps the estate from adding a paper control.
controls-census-gate:
	python3 tools/controls_census.py --fail-on-undiscriminating

# A CronJob mounts its script from a ConfigMap generated inside a kustomize base, which (given
# kustomize's default load-restrictor) requires a COPY of the canonical tools/ script. This
...
validate-argocd-source-sovereignty:
	python3 -m pip install --user pyyaml >/dev/null 2>&1 || true
	python3 tools/verify_argocd_source_sovereignty.py
...
sophos-reasoner-smoke:
	cd apps/sophos-reasoner && test -d .venv || python3 -m venv .venv
	cd apps/sophos-reasoner && . .venv/bin/activate && python -m pip install --upgrade pip pytest -r requirements.txt && PYTHONPATH=src pytest -q tests
...
validate-vector-chunk-contract:
	python3 tools/validate_vector_chunk_contract.py
...
validate-evidence-cover-graph:
	python3 tools/validate_evidence_cover_graph.py

check-cover-sufficiency:
	python3 tools/check_cover_sufficiency.py \
		contracts/evidence/sufficiency-check/check_cover_sufficiency_passes_floor.json \
		--tier T1 \
		--fixture-dir contracts/evidence/sufficiency-check
	pytest -q tools/tests/test_check_cover_sufficiency.py
