...
def create_thread(body: ThreadIn) -> dict[str, object]:
    ...
    record = {'thread_id': body.thread_id,'document_id': body.document_id,'thread_type': body.thread_type,'semantic_unit_ref': body.semantic_unit_ref,'status': 'OPEN','version_id': None,'receipt_ref': None,'created_at': now,'updated_at': now}
...
def create_suggestion(body: SuggestionIn) -> dict[str, object]:
    now = datetime.now(timezone.utc).isoformat()
    record = {'suggestion_id': body.suggestion_id,'document_id': body.document_id,'semantic_unit_ref': body.semantic_unit_ref,'before_ref': body.before_ref,'after_ref': body.after_ref,'status': 'PROPOSED','version_id': None,'receipt_ref': None,'created_at': now,'updated_at': now}
